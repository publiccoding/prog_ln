...
---*******************************************************************************
-- ORACLE Tablespaces 
create tablespace thimma1 datafile 'thimma1.dat' size 100M reuse autoextend on next 10M maxsize 200M;
...
GRANT CONNECT TO thimma;
GRANT CONNECT, RESOURCE, DBA TO thimma;
GRANT CREATE SESSION to thimma;
GRANT UNLIMITED TABLESPACE TO thimma;
GRANT CONNECT, RESOURCE, DBA TO thimma;
...
drop user thimma cascade;
create user thimma identified by thimma default tablespace thimma;
...
GRANT CREATE TABLE TO thimma WITH ADMIN OPTION;
GRANT AQ_ADMINISTRATOR_ROLE TO THIMMA WITH ADMIN OPTION; 
GRANT DBA TO THIMMA WITH ADMIN OPTION; 
ALTER USER THIMMA DEFAULT ROLE ALL; 
GRANT ALTER ANY INDEX TO THIMMA; 
GRANT ANALYZE ANY TO THIMMA; 
GRANT CREATE ANY TABLE TO THIMMA; 
GRANT CREATE MATERIALIZED VIEW TO THIMMA; 
GRANT CREATE SESSION TO THIMMA; 
GRANT CREATE TABLE TO THIMMA; 
GRANT DROP ANY TABLE TO THIMMA; 
GRANT EXECUTE ANY PROCEDURE TO THIMMA; 
GRANT EXECUTE ANY PROGRAM TO THIMMA; 
GRANT GLOBAL QUERY REWRITE TO THIMMA; 
GRANT SELECT ANY DICTIONARY TO THIMMA; 
GRANT SELECT ANY TABLE TO THIMMA; 
GRANT UNDER ANY TABLE TO THIMMA; 
GRANT UNLIMITED TABLESPACE TO THIMMA WITH ADMIN OPTION; 
...
CREATE OR REPLACE PROCEDURE totalCustomer(p_id  number, p_name OUT varchar2, p_addr OUT varchar2) as 
BEGIN 
   SELECT name,addr into p_name,p_addr FROM emp_data where id = p_id; 
EXCEPTION 
WHEN OTHERS THEN
...
DECLARE 
   a number :=100 ;
   b varchar2(20);
   c emp_data.addr%TYPE;
BEGIN 
   totalCustomer(a,b,c); 
   DBMS_OUTPUT.PUT_LINE('Total no. of Customers: '||a||'  '||c||' '|| b); 
END; 


select * from dba_objets where object_name ='totalCustomers';
desc dba_objects

create type emp_obj is object (empno number, ename varchar2(10), eaddr varchar2(20));
...
create or replace function all_emps return emp_tab
  is
     ...
     n integer := 0;
  begin
     for r in (select id, name, addr from emp_data)
     loop
        l_emp_tab.extend;
        n := n + 1;
       l_emp_tab(n) := emp_obj(r.id, r.name, r.addr);
     end loop;
     return l_emp_tab;
  end;
...
CREATE OR REPLACE PACKAGE BODY personnel AS
  ...
  -- get salary
  FUNCTION get_salary(n_emp_id NUMBER) RETURN VARCHAR2 IS
    n_salary VARCHAR2(20);
  BEGIN
    SELECT addr
    INTO n_salary
    FROM emp_data
