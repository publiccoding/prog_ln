...
CREATE PUBLIC DATABASE LINK dblink CONNECT TO thimma identified by thimma using  testdb;
...
insert into trigger_test values(1,'thimma');

CREATE TABLE emp_data_trig
( id number,
  ename varchar2(30),
  addr varchar2(40),
  salary number
...
CREATE OR REPLACE TRIGGER orders_before_insert_trig
BEFORE INSERT
   ON orders
   FOR EACH ROW
DECLARE
...
END;
insert into orders values(10,10,sysdate,'thimmma');

--***************************************************************************************

grant create session, create procedure to thimma identified by thimma;
...
PROCEDURE empselect(x IN OUT number) IS 
BEGIN 
  x := x * x; 
END;  
BEGIN 
